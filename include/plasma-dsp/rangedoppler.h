...
//  */
// template <typename T>
// std::vector<std::complex<T>> MatchedFilterResponse(
//     std::vector<std::complex<T>> &in, std::vector<std::complex<T>> &ref) {
//   return conv(in, ref);
// }
 ...
 */
MatrixXcd MatchedFilterResponse(MatrixXcd in, VectorXcd ref) {
  auto conv_length = in.rows() + ref.size() - 1;
  int num_pulses = in.cols();
  MatrixXcd in_zeros = MatrixXcd::Zero(conv_length-in.rows(),num_pulses);
  MatrixXcd in_mat(conv_length, num_pulses);
  in_mat << in, in_zeros;
  VectorXcd ref_zeros = VectorXcd::Zero(conv_length-ref.size());
...
//  */
// template <typename T>
// Matrix2D<std::complex<T>> MatchedFilterResponse(Matrix2D<std::complex<T>>
// &in,
...
//       Matrix<std::complex<T>, Dynamic, Dynamic>::Zero(conv_length,
//       num_pulses);
...
//   auto out = Matrix<std::complex<T>, Dynamic, Dynamic>(conv_length,
//   num_pulses);
//   // Compute the FFT for each row, multiply by the transformed reference
//   vector,
...
//           MatchedFilterResponse(pulses, ref).data(), conv_length,
//           num_pulses);
//   // Compute the FFT across each row
//   FFT<T> fft;
//   auto out = Matrix<std::complex<T>, Dynamic, Dynamic>(conv_length,
//   num_pulses); for (int i_row = 0; i_row < conv_length; i_row++)
