...
def richardson_lucy(RD: jnp.ndarray,
                    P: jnp.ndarray,
                    ) -> jnp.ndarray:
  J = RD
  for i in range(1):
    frac = RD / jnp.fft.ifft2(jnp.fft.fft2(J) * jnp.fft.fft2(P, J.shape))
    J = J * (jnp.fft.ifft2(jnp.fft.fft2(frac) * jnp.fft.fft2(P, J.shape)))
  return J
...
if __name__ == '__main__':
  ...
  # Simulate the pulses with a delay and/or doppler shift
  ...
  # For testing, use the same RDM for both arguments in the RL-deconvolution
  start = time.time()
  richardson_lucy(rdm, rdm)
